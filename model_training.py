...
Now loads data from CSV instead of yfinance.
"""
...
def prepare_features(features: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        sns.heatmap(features.corr(), cmap='coolwarm', center=0, annot=False, square=True)
...
def split_data(data: pd.DataFrame, features: pd.DataFrame) -> Tuple:
    """Split data into train, test, and validation sets (60%, 20%, 20%)."""
    ...
    for symbol, data in analyzer.crypto_data.items():
        ...
        try:
            ...
            for model in models:
                ...
                train_metrics = ml_model_manager.train_model(model, train_features, train_data)
                test_metrics = ml_model_manager.evaluate_model(model, test_features, test_data)
                val_metrics = ml_model_manager.evaluate_model(model, val_features, val_data)
            ...
            print(f"\nBest model for {symbol}:", best_model['model_name'])
            print("Best validation metrics:", best_model['validation_metrics'])
