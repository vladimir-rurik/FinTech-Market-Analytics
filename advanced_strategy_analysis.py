"""
Advanced trading strategy analysis script (now loads from CSV).
"""
...
def calculate_performance_metrics(returns: pd.Series) -> dict:
    ...
    win_rate = winning_days.mean() if len(returns) > 0 else 0.0

    # Sortino
    negative_returns = returns[returns < 0]
    if len(negative_returns) > 0:
        sortino_ratio = annual_return / (negative_returns.std() * np.sqrt(252))
    else:
        sortino_ratio = 0
...
def main():
    ...
    # Process each asset in crypto_data
    for symbol, data in analyzer.crypto_data.items():
        ...
        try:
            ...
            results.to_csv(f'results/{symbol}_advanced_strategy.csv', index=False)
